...
async function analyze(filePath) {
  try {
    ...
    // Skip analysis for non-supported files but include them in the dependency graph
    if (['.html', '.css', '.svg', '.md'].includes(fileExtension)) {
      return {
        imports: [],
        contexts: [],
        props: [],
    ...
    // Parse the code to AST with enhanced support for different file types
    ...
    if (fileExtension === '.vue' || fileExtension === '.svelte') {
      // For non-standard file types, we'll extract script tags later
      // Currently just basic support
    }
    
    // Parse with appropriate options
    ...
    // Return a minimal result when analysis fails
    return {
      imports: [],
      contexts: [],
      props: [],
