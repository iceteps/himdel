...
export async function analyzeCodebase(dir, options) {
  ...
  const excludePatterns = options.exclude ? options.exclude.split(',').map(pattern => pattern.trim()) : [];
  const ignorePatterns = excludePatterns.map(pattern => `**/${pattern}/**`);
  
  logger.debug(`Excluding patterns: ${JSON.stringify(ignorePatterns)}`);
  
  // Use normalized path in glob pattern with ignore patterns
  const pattern = `${normalizedDir}/**/*.{js,jsx,ts,tsx}`;
  ...
  if (files.length === 0) {
    logger.warn('No files found to analyze. Check your --dir path and make sure it contains JS/TS files.');
  }
